...
// Styling updated for document-like reading experience.
// ──────────────────────────────────────────────────────────────────────────────
...
function InlineText({ text }: { text: string }) {
  ...
  const patterns = [
    ...
    {
      re: /\[(.+?)\]\((.+?)\)/,
      render: (_m: string, label: string, href: string) => (
...
function LatexRenderer({ expression, display }: { expression: string; display: boolean }) {
  ...
  return (
    <div
      ref={ref}
      className={display
        ? 'my-5 flex justify-center overflow-x-auto text-[var(--foreground)] py-3'
        : 'inline-block text-[var(--foreground)]'
      }
    />
  );
}
...
function SvgRenderer({ markup, caption }: { markup: string; caption?: string }) {
  ...
  return (
    <div className="my-5 flex flex-col items-center gap-2">
      <div
        ref={ref}
        className="w-full max-w-md overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--background-secondary)] p-4 flex justify-center [&_svg]:max-w-full [&_svg]:h-auto"
      />
      {caption && <p className="text-xs text-[var(--foreground-muted)] italic text-center">{caption}</p>}
    </div>
  );
}
  ...
  return (
    <div className="my-5 flex flex-col items-center gap-2">
      {svgCode ? (
        <div
          ref={ref}
          className="w-full max-w-full overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--background-secondary)] p-5 flex justify-center [&_svg]:max-w-full [&_svg]:h-auto"
          dangerouslySetInnerHTML={{ __html: svgCode }}
        />
      ) : (
        <div
          ref={ref}
          className="w-full max-w-full overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--background-secondary)] p-5 flex justify-center items-center"
        >
          <span className="text-xs text-[var(--foreground-muted)]">Rendering diagram...</span>
        </div>
      )}
      {caption && <p className="text-xs text-[var(--foreground-muted)] italic text-center">{caption}</p>}
    </div>
      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="bg-[var(--background-secondary)]">
            {headers.map((h, i) => (
              <th key={i} className="px-4 py-3 text-left font-semibold text-[var(--foreground)] border-b border-[var(--border)] text-xs uppercase tracking-wide">
                {h}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {body.map((row, ri) => (
            <tr key={ri} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--background-secondary)]/50 transition-colors">
              {row.map((cell, ci) => (
                <td key={ci} className="px-4 py-3 text-[var(--foreground-secondary)]">
...
export default function BlockPreview({ block }: BlockPreviewProps) {
  ...
  switch (block.type) {
    ...
    case 'paragraph':
      return (
        <p className="text-base text-[var(--foreground-secondary)] leading-relaxed my-4">
          {block.text.split('\n').map((line, i) => (
            <span key={i}>
              {i > 0 && <br />}
      ...
      return (
        <div className="my-5 flex flex-col items-center gap-2">
          ...
          <img
            ...
            className="max-w-full max-h-80 rounded-2xl border border-[var(--border)] object-contain"
          />
          {block.caption && <p className="text-xs text-[var(--foreground-muted)] italic">{block.caption}</p>}
        </div>
      );
    ...
    case 'link':
      return (
        <a
          ...
          className="my-4 flex items-start gap-3 p-4 rounded-2xl border border-[var(--border)] bg-[var(--background-secondary)] hover:border-[var(--primary)]/40 hover:bg-[var(--primary)]/5 transition-all duration-200 group"
        >
          <div className="mt-0.5 text-[var(--primary)] shrink-0">
            <ExternalLink className="h-4 w-4" />
          </div>
          <div className="min-w-0">
            <p className="font-medium text-[var(--foreground)] group-hover:text-[var(--primary)] transition-colors text-base">{block.label}</p>
            {block.description && <p className="text-xs text-[var(--foreground-muted)] mt-0.5 truncate">{block.description}</p>}
            <p className="text-xs text-[var(--primary)]/60 mt-0.5 truncate">{block.url}</p>
          </div>
        </a>
      );
    ...
    case 'code':
      ...
      return (
        <div className="my-5">
          <div className="flex items-center justify-between px-4 py-2.5 bg-slate-800 rounded-t-2xl border border-slate-700">
          ...
          <pre className="bg-slate-900 rounded-b-2xl p-5 overflow-x-auto border border-slate-700 border-t-0">
            <code className="text-sm font-mono text-slate-200 whitespace-pre leading-relaxed">{block.code}</code>
          </pre>
        </div>
      );
