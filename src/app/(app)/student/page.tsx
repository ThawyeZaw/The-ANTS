    ...
    <div className="space-y-6">
      ...
      <div className="card-standard p-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <Link href="/library" className="text-sm font-medium text-primary hover:text-primary-hover transition-colors flex items-center gap-1.5 focus-ring rounded-md">
        ...
        {savedNotes.length === 0 ? (
          <div className="bg-background-secondary/50 border border-dashed border-border rounded-2xl p-8 text-center">
            ...
            <p className="text-xs text-foreground-muted mt-1.5 max-w-sm mx-auto">Browse the library and save notes to see them here.</p>
            <Link href="/library" className="inline-flex items-center gap-1.5 mt-4 text-sm font-medium text-primary hover:text-primary-hover transition-colors focus-ring rounded-md">
      ...
      {stats && stats.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {stats.map((stat) => (
            <div
              key={stat.key}
              className="card-standard p-4 hover:-translate-y-0.5"
            >
              <div className={cn('inline-flex p-1.5 rounded-xl mb-2', colorMap[stat.color] || 'text-foreground bg-foreground/10')}>
