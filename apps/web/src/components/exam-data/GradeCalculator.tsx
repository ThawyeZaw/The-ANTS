...
export default function GradeCalculator() {
  ...
  return (
    <div className="rounded-3xl border border-border bg-background-card p-4 sm:p-6 md:p-8 shadow-xs space-y-6 sm:space-y-8 max-w-5xl mx-auto">
      {/* Top Header */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-border pb-6">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto">
          <Link
            ...
            className="flex-1 sm:flex-initial inline-flex items-center justify-center gap-1.5 rounded-2xl border border-border bg-background-secondary px-3.5 sm:px-4 py-2.5 text-xs font-bold text-foreground hover:text-primary transition-colors"
          >
          ...
          <button
            onClick={resetAll}
            className="inline-flex items-center justify-center gap-2 rounded-2xl border border-border bg-background-secondary px-3.5 sm:px-4 py-2.5 text-xs font-semibold text-foreground hover:bg-background-secondary/80 transition-colors"
          >
      ...
      <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
        {[
          ...
          <button
            ...
            disabled={!s.enabled}
            className={cn(
              'py-3 px-2 sm:py-3.5 sm:px-4 rounded-2xl border transition-all text-center',
              currentStep === s.n
      ...
      {currentStep === 2 && (isEdexcelIal ? selectedSubject : activePreset) && (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {paperResults.map((pr) => {
              const inputId = `mark-input-${pr.baseKey}`;
              return (
                <div
                  key={pr.baseKey}
                ...
                >
                  {/* Row 1: Paper Title & Variant Switcher */}
                  <div className="flex items-center justify-between gap-2">
                    <label
                      htmlFor={inputId}
                      className="text-xs font-bold text-foreground line-clamp-1 leading-snug cursor-pointer select-none"
                      title={pr.displayName || pr.name}
                    >
                      {pr.displayName || pr.name}
                    </label>
                    {pr.availableVariants.length > 1 && (
                      <div className="inline-flex items-center gap-0.5 bg-background-card border border-border rounded-lg p-0.5 shrink-0">
                        {pr.availableVariants.map((v) => (
                          <button
                            key={v}
                            type="button"
                            onClick={() => setPaperVariants((prev) => ({ ...prev, [pr.baseKey]: v }))}
                            className={cn(
                              'px-2 py-0.5 sm:px-1.5 sm:py-0.5 rounded text-[10px] font-bold font-mono transition-colors',
                              pr.activeVariant === v
                                ? 'bg-primary text-white shadow-xs'
                                : 'text-foreground-muted hover:text-foreground'
                            )}
                            aria-label={`Select variant ${v} for ${pr.displayName || pr.name}`}
                          >
                            v{v}
                          </button>
                        ))}
                  ...
                  <div className="flex items-center justify-between gap-3 pt-0.5">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <span
                        ...
                        aria-label={pr.filled ? `Grade ${pr.paperGrade}` : 'No grade calculated'}
                      >
          ...
          <div className="rounded-2xl border border-border bg-background-card p-4 sm:p-5 space-y-3">
            <div className="flex items-start justify-between gap-4">
              ...
              <div
                className={cn(
                  'w-14 h-14 rounded-xl flex items-center justify-center text-2xl font-extrabold border shrink-0 shadow-xs transition-colors',
            ...
            {(calc.anyFilled && calc.aStarNotes && calc.aStarNotes.length > 0) || ('boundariesMessage' in calc && calc.boundariesMessage) ? (
              <div className="space-y-1 pt-2 border-t border-border/40">
                {calc.anyFilled && calc.aStarNotes && calc.aStarNotes.length > 0 && (
                  <p className="text-[11px] text-foreground-muted leading-relaxed">{calc.aStarNotes.join(' ')}</p>
                )}
                {'boundariesMessage' in calc && calc.boundariesMessage && (
                  <p className="text-[11px] text-foreground-muted leading-relaxed">{calc.boundariesMessage as string}</p>
                )}
              </div>
            ) : null}
          </div>
          ...
          <div className="flex items-center justify-between gap-2.5 sm:gap-4 pt-4">
            <button
              type="button"
              onClick={() => setCurrentStep(1)}
              className="inline-flex items-center justify-center gap-1.5 sm:gap-2 flex-1 sm:flex-initial rounded-2xl border border-border bg-background-secondary px-3.5 sm:px-5 py-2.5 sm:py-3 text-xs font-bold text-foreground hover:bg-background-secondary/80 transition-colors"
            >
            ...
            <Link
              href={selectedSubject ? `/past-papers?subject=${selectedSubject}` : '/past-papers'}
              className="inline-flex items-center justify-center gap-1.5 sm:gap-2 flex-1 sm:flex-initial rounded-2xl bg-primary px-3.5 sm:px-5 py-2.5 sm:py-3 text-xs font-bold text-white shadow-xs hover:bg-primary-hover transition-colors text-center"
            >
