        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="block text-xs font-medium uppercase tracking-widest text-[var(--foreground-secondary)]"><BookOpen className="w-3.5 h-3.5 inline mr-1.5 text-primary" />Curriculum</label>
            ...
            <div className="space-y-1.5">
              <label className="block text-xs font-medium uppercase tracking-widest text-[var(--foreground-secondary)]"><Layers className="w-3.5 h-3.5 inline mr-1.5 text-primary" />Subject</label>
          ...
          <div className="flex justify-end gap-3 pt-4"><button type="button" onClick={() => setCurrentStep(2)} disabled={!canGoStep2} className="flex items-center gap-2 rounded-xl bg-primary px-5 py-3 text-xs font-medium text-primary-foreground shadow-lg disabled:opacity-40 disabled:cursor-not-allowed transition-all">Next: Enter Marks <ArrowRight className="w-3.5 h-3.5" /></button></div>
        </div>
      )}
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="bg-[var(--background-card)] rounded-3xl p-6 shadow-[var(--shadow-glow)] relative overflow-hidden border border-[var(--border)] mt-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
              <div className="space-y-4">
                <div><span className="text-[10px] font-medium text-[var(--foreground-muted)] uppercase tracking-widest">Overall Calculation</span></div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-xl border border-[var(--border)] bg-[var(--background-secondary)] p-3">
                    ...
                    <span className="text-lg font-semibold text-[var(--foreground)]">{calc.anyFilled ? `${calc.totalRaw} / ${calc.maxRaw}` : '—'}</span>
                  </div>
                  <div className="rounded-xl border border-[var(--border)] bg-[var(--background-secondary)] p-3">
                    ...
                    <span className="text-lg font-semibold text-[var(--foreground)]">{calc.anyFilled ? (activePreset.is_modular ? `${calc.totalRaw}` : `${calc.weighted} / ${calc.maxWeight}`) : '—'}</span>
                  </div>
                </div>
              </div>
              <div className="flex flex-col items-center justify-center text-center border-t md:border-t-0 md:border-l border-[var(--border)] pt-4 md:pt-0 md:pl-6">
                <span className="text-[10px] font-medium text-[var(--foreground-muted)] uppercase tracking-widest mb-2">{activePreset.is_modular ? 'Estimated Overall %' : 'Estimated PUM Score'}</span>
                ...
                <span className="text-[10px] font-medium text-[var(--foreground-muted)] uppercase tracking-widest mt-4 mb-2">Estimated Grade</span>
          ...
          {calc.anyFilled && (
            <div className="rounded-2xl border border-[var(--border)] bg-[var(--background-card)] p-5">
              <h4 className="text-[10px] font-medium uppercase tracking-wider text-[var(--foreground-secondary)] mb-3">Overall Grade Boundaries</h4>
                    ...
                    {activePreset.grade_boundaries.map(b => { 
                      ...
                      return (
                        <tr key={b.grade} className={`border-b border-[var(--border)] ${isAch ? 'bg-primary/10' : ''}`}>
                          <td className={`py-2 font-medium ${isAch ? 'text-primary' : 'text-[var(--foreground)]'}`}>{b.grade} {isAch && <ChevronRight className="w-3 h-3 inline text-primary" />}</td>
                          <td className={`py-2 text-right tabular-nums ${isAch ? 'text-primary font-medium' : 'text-[var(--foreground-secondary)]'}`}>{b.min_mark}</td>
                        </tr>
                      ); 
                    })}
                  </tbody>
